...
// ============================================================
// VERIFY OTP — actual INSERT happens server-side after this
// ============================================================
function verifyOTP() {
    ...
    Swal.fire({
        title:'Verifying…', allowOutsideClick:false, showConfirmButton:false,
        didOpen: () => Swal.showLoading()
    });

    $.post(
        baseUrl + 'api/patients.php?action=verify',
        JSON.stringify({ email, otp }),
        function (r) {
            Swal.close();
            if (r.success) {
                clearInterval(otpTimer);
                // Hide OTP modal, refresh table — patient row now exists
                ...
                Swal.fire({
                    ...
                    text:'Patient has been added and account is now active.',
                    timer:2500, showConfirmButton:false
                });
            } else {
                if (r.can_resend) {
                    Swal.fire({
                        icon:'warning', title:'OTP Expired',
                        text:r.message,
...
function resendOTP() {
    const email = $('#otpEmail').text().trim();
    $.post(
        baseUrl + 'api/patients.php?action=resend',
...
function upgradeAccount() {
    ...
    $.post(baseUrl + 'api/patients.php?action=upgrade', JSON.stringify(data), function (r) {
        if (r.success) {
...
function printRxFromPatientView(index) {
    ...
    win.document.write('<div style="text-align:center;border-bottom:1px dashed #ccc;padding-bottom:7px;margin-bottom:7px;"><div style="font-size:12px;font-weight:900;color:#0d9488;">👁 EYECORE OPTICAL CLINIC</div><div style="font-size:8px;color:#64748b;">EyeCore PH &middot; eyecore.com</div></div>');
