...
<head>
    ...
    <style>
        ...
        * {
        ...
        .location-badge i.fa-check-circle {
            color: var(--success);
        }

        .location-badge i.fa-exclamation-triangle {
            color: var(--danger);
        }

        .location-badge i.fa-spinner {
            color: var(--primary);
        }
        ...
        .clinic-image-placeholder i {
            font-size: 48px;
            color: var(--primary);
            opacity: 0.6;
        }

        .clinic-image-placeholder span {
            font-size: 12px;
        ...
        .clinic-address i, .clinic-contact i {
            color: var(--primary);
            width: 16px;
        }
        ...
        .btn-view, .btn-view-map, .btn-book {
            flex: 1;
            padding: 10px 16px;
            border-radius: var(--radius-md);
            font-size: 12px;
        ...
        .loading-location i, .no-results i {
            font-size: 60px;
            color: var(--text-muted);
            margin-bottom: 20px;
            opacity: 0.5;
        ...
        .loading-location h3, .no-results h3 {
            font-size: 20px;
            color: var(--text-primary);
            margin-bottom: 10px;
        }

        .loading-location p, .no-results p {
            color: var(--text-secondary);
            max-width: 400px;
            margin: 0 auto;
        }
...
<body>
    ...
    <script>
        ...
        // Clinic data from PHP - FIXED JSON ENCODING
            ...
            if (filteredClinics.length === 0) {
                list.innerHTML = `
                    <div class="no-results">
                        ...
                        <p>Try adjusting your filters or increasing the radius</p>
                    </div>
                `;
                    ...
                    <div class="clinic-card" data-id="${clinic.id}">
                        ...
                        <div class="clinic-info-main">
                            ...
                            <div class="clinic-meta">
                                ...
                                <span class="clinic-distance">
                                    <i class="fas fa-location-arrow"></i> ${distanceText}
                                </span>
                            </div>
                            
                            <div class="travel-time">
                                <span class="travel-time-item">
                                    <i class="fas fa-car"></i> ${carTime} by car
                                </span>
                                <span class="travel-time-item">
                                    <i class="fas fa-walking"></i> ${footTime} on foot
                                </span>
                            </div>
                            ...
                            <div class="clinic-contact">
                                <i class="fas fa-phone"></i> ${escapeHtml(clinic.contact)}
                            </div>
                        </div>
                        ...
                        <div class="clinic-info-side">
                            ...
                            <div class="card-actions">
                                <a href="clinic-details.php?id=${clinic.id}" class="btn-view">
                                    <i class="fas fa-eye"></i> View
                                </a>
                                <button class="btn-view-map" onclick="focusOnMap(${clinic.id})">
                                    <i class="fas fa-map-marker-alt"></i> Map
                                </button>
                                <a href="book-appointment.php?clinic_id=${clinic.id}" class="btn-book">
            ...
            displayedClinics.forEach(clinic => {
                if (clinic.latitude && clinic.longitude) {
                    ...
                    const markerIcon = L.divIcon({
                        html: `
                            <div style="position: relative;">
                                <div style="background-color: ${clinic.type_color}; width: 24px; height: 24px; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center; color: white; font-size: 12px;">
                                    <i class="fas fa-eye"></i>
                                </div>
                                ${clinic.is_open ? 
                                  '<div style="position:absolute; top:-2px; right:-2px; background:var(--success); width:10px; height:10px; border-radius:50%; border:2px solid white; box-shadow:0 2px 5px rgba(0,0,0,0.2);"></div>' : 
                                  '<div style="position:absolute; top:-2px; right:-2px; background:var(--danger); width:10px; height:10px; border-radius:50%; border:2px solid white; box-shadow:0 2px 5px rgba(0,0,0,0.2);"></div>'}
                            </div>
                        `,
                        iconSize: [30, 30],
                        iconAnchor: [15, 15],
                    ...
                    marker.bindPopup(`
                        ...
                        <span style="color: ${clinic.is_open ? 'var(--success)' : 'var(--danger)'}">
                            <i class="fas ${clinic.is_open ? 'fa-door-open' : 'fa-door-closed'}"></i>
                            ${clinic.is_open ? 'Open Now' : 'Closed'}
                        </span><br>
                        <i class="fas fa-location-arrow"></i> ${clinic.distance ? clinic.distance.toFixed(1) + ' km' : 'Unknown'}<br>
                        <i class="fas fa-car"></i> ${carTime} by car
                    `);
        ...
        function focusOnMap(clinicId) {
            ...
            if (clinic && clinic.latitude && clinic.longitude) {
                ...
                const marker = clinicMarkers.find(m => 
